...
{
    char buffer[256];
    strncpy_s(buffer, msg1, sizeof(buffer));
    strncat_s(buffer, msg2, sizeof(buffer));
    return buffer;
}
...
bool checkSig(uintptr_t& sig, const char* sigmsg)
{
    ...
    {
        //Pattern Not Found
        MessageBoxA(0, charcmb(ENC("Failed to find Function: "), sigmsg), ENC("Cobalt Fatal Error!"), MB_ICONERROR);
        return FALSE;
    }
    else
    {
        DebugLog(charcmb(ENC("Found Function: "), sigmsg));
        return TRUE;
    }
}
